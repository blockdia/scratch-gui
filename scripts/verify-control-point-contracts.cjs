...
const load = (filename, mocks, globals = {}) => {
    ...
    vm.runInNewContext(code, {
        ...
        // Resolve dependencies dynamically so the test loader can substitute mocks.
...
let lastMetadata;
geometry.props.vm.setComponentMetadata = (targetId, metadata) => {
    commits++; lastMetadata = metadata;
};
...
const keyboard = new Keyboard({
    ...
    changeMode: () => {
        throw new Error('paint mode changed');
    },
    onUpdateImage: () => {
        throw new Error('costume changed');
    }
});
