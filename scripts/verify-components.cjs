/* eslint-env browser */
/* global vm, sliderTarget, paper, progressTarget, ordinaryTarget, toggleTarget, buttonTarget */
...
(async () => {
    ...
    try {
        ...
        const add = async type => {
            ...
            await page.getByRole('button', {name: /Choose a Sprite|选择一个角色/, exact: true}).first()
                .hover();
        ...
        assert.equal(await popup.getByRole('spinbutton').first()
        ...
        await popup.waitFor({state: 'hidden'});
        const previousStageSize = await page
            .locator('[class*="stage-header_stage-size-toggle-group"] button[aria-pressed="true"]')
        ...
        await page.locator('[class*="stage-header_stage-size-toggle-group"]').getByRole('button',
            {name: previousStageSize, exact: true})
            .click();
        await page.waitForFunction(() => document.querySelector('section [class*="component-panel_type_"]'));
            ...
            for (const direction of [90, 0]) {
                for (const mirror of [false, true]) {
                    ...
                    for (const value of [25, 75]) {
                        ...
                        const hit = p => renderer.drawableTouching(id,
                            ((p[0] / 480) + 0.5) * renderer.canvas.clientWidth,
                            (0.5 - (p[1] / 360)) * renderer.canvas.clientHeight);
                        row.push({stripe: sample(...left),
                            clipped: sample(...right),
                            leftHit: hit(left),
                            rightHit: hit(right)});
                    }
                    rows.push(row);
        ...
        for (const [type, x, y] of [['button', 100, 70], ['toggle', 100, -40], ['progress', -80, -80]]) {
            ...
            await page.evaluate(({
                ...
                window[`${componentType}Target`] = vm.editingTarget;
                vm.editingTarget.setXY(targetX, targetY);
            }, {
                type,
                x,
                y
        ...
        const paletteResults = await page.evaluate(() => {
            ...
            return vm.runtime.targets.map(target => {
                ...
                return {self,
                    expected: (target.component ? expected[target.component.type] : []).map(x => `components_${x}`),
        ...
        const declaredMenus = await page.evaluate(() => {
            ...
            const args = Object.keys(block.json).filter(key => key.startsWith('args'))
                .flatMap(key => block.json[key]);
        ...
        await page.evaluate(() => {
            ...
            create('component-menu-regression', 'components_targetProperty', null, {
                TARGET: {name: 'TARGET',
                    block: 'component-menu-regression-target',
                    shadow: 'component-menu-regression-target'}
        ...
        await page.waitForFunction(() => {
            ...
            const block = workspace && workspace.getTopBlocks(false)
                .find(item => item.type === 'components_targetProperty');
            return block && block.getField('PROPERTY').getOptions()
                .every(option => option[1] !== 'step');
        });
        await page.evaluate(() => vm.setEditingTarget(sliderTarget.id));
        await page.waitForFunction(() => {
            const blockly = window.Blockly || window.ScratchBlocks;
            const block = blockly && blockly.getMainWorkspace().getBlockById('component-menu-regression');
            return block && block.getFieldValue('PROPERTY') === 'step' &&
                block.getField('PROPERTY').getText() !== 'step';
        });
        await page.evaluate(() => {
            const blockly = window.Blockly || window.ScratchBlocks;
            const block = blockly.getMainWorkspace().getBlockById('component-menu-regression');
            block.getInputTargetBlock('TARGET').getField('numericTargets')
                .setValue(progressTarget.getName());
        });
        await page.waitForFunction(() => {
            const blockly = window.Blockly || window.ScratchBlocks;
            const block = blockly && blockly.getMainWorkspace().getBlockById('component-menu-regression');
        ...
        await page.waitForFunction(() => {
            ...
            const main = blockly && blockly.getMainWorkspace();
            const workspace = main && main.getFlyout().getWorkspace();
            return workspace && workspace.getTopBlocks(false)
                .some(block => block.type === 'components_whenStateChanged');
        });
        ...
        for (const compilerEnabled of [false, true]) {
            ...
            assert.equal(Number(execution.value), compilerEnabled ? 73 : 62);
            assert.equal(Boolean(execution.compiled), compilerEnabled);
            console.log('PASS unified component setter and reporter', compilerEnabled ? 'compiled' : 'interpreted');
        }
        for (const compilerEnabled of [false, true]) {
            const execution = await page.evaluate(enabled => {
                vm.stopAll();
                ...
                const block = (name, opcode, parent, next, inputs = {}, fields = {}, shadow = false) =>
                    owner.blocks.createBlock({id: id(name),
                        opcode,
                        parent: parent && id(parent),
                        next: next && id(next),
                        inputs,
            ...
            assert.equal(Boolean(execution.compiled), compilerEnabled);
            console.log('PASS cross-component dropdown and reporter input',
                compilerEnabled ? 'compiled' : 'interpreted');
        }
        await page.evaluate(() => {
            sliderTarget.componentController.setProperties({
                value: 0
        ...
        assert.ok(await page.evaluate(() => vm.runtime.targets.filter(t => t.component)
            .every(t => t.getCostumes().every(c => !c.broken))));
